  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        console.log(
          '[use-task-tags-data] Fetched categories from tasks:',
          categories
        );
        ...
        if (statusCode === 422 || statusCode === 404) {
          console.log(
            '[use-task-tags-data] Categories endpoint not available, using Category tag group instead'
          );
        } else {
      ...
      const fetchAllTags = async () => {
        ...
        for (const group of dbTagGroups) {
          try {
            ...
            console.error(
              `[use-task-tags-data] Error fetching tags for group ${group.id}:`,
              err
            );
          }
        }
