...
test.describe('Home screen timer', () => {
  ...
  test('reflects default values and supports adjustments', async ({ page }) => {
    ...
    await expect
      .poll(() => consoleLogs.some((log) => log.includes(actions.recommendLog)))
      .toBeTruthy();
  });

  test('validates "Other" place input with character limit and warning', async ({
    page,
    browserName,
  }) => {
    ...
    test.skip(
      browserName === 'webkit',
      'Filter modal not reliably rendered in WebKit in CI.'
    );
